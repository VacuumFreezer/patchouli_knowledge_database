import { createRequire as __patchouliCreateRequire } from "node:module"; const require = __patchouliCreateRequire(import.meta.url);
...
${indent}${text.slice(0, end2)}`;
          ...
          if (mode === FOLD_QUOTED && escapedFolds[fold2])
            res += `${text[fold2]}\\`;
          res += `
${indent}${text.slice(fold2 + 1, end2)}`;
        }
      }
      return res;
    }
...
// src/core/errors.ts
var PatchouliError = class extends Error {
  code;
  details;
...
var WINDOWS_INVALID_COMPONENT = /[<>:"|?*\u0000-\u001F\u007F]/u;
var WINDOWS_INVALID_FILENAME = /[<>:"/\\|?*\u0000-\u001F\u007F]+/gu;
...
function normalizeRelativePath(value, fieldName = "path") {
  ...
  if (path.isAbsolute(trimmed) || path.win32.isAbsolute(trimmed) || path.posix.isAbsolute(trimmed) || /^[A-Za-z]:/u.test(trimmed)) {
  ...
  for (const segment of segments) {
    if (segment.length === 0 || segment === "." || segment === ".." || segment.endsWith(" ") || segment.endsWith(".") || WINDOWS_INVALID_COMPONENT.test(segment)) {
...
async function resolveContainedPath(root, relativePath, options = {}) {
...
async function validateVaultDirectory(vaultPath) {
...
async function ensureContainedDirectory(root, relativePath) {
...
function sanitizeTitleToFilename(title) {
  ...
  let stem = title.normalize("NFKC").replace(WINDOWS_INVALID_FILENAME, "-").replace(/\s+/gu, " ").replace(/-+/gu, "-").replace(/(?:-\s*){2,}/gu, "-").replace(/^[.\s-]+|[.\s-]+$/gu, "");
  if (stem.length === 0) {
...
function toPosixRelativePath(root, candidate) {
...
// src/core/cards.ts
function normalizeInline(value, field, allowEmpty = false) {
...
function normalizeMarkdown(value, field, allowEmpty = false) {
  if (typeof value !== "string") {
    throw new PatchouliError("VALIDATION_ERROR", `${field} must be a string.`, { field });
  }
...
function uniqueTrimmed(values, field) {
...
function normalizeSource(source, index) {
  ...
  if (source.url !== void 0 && source.url.trim().length > 0) {
    let parsed;
    try {
...
function renderSources(sources) {
  ...
  return sources.map((source) => {
    ...
    return source.url ? `- ${prefix}[${escapeMarkdownLabel(source.label)}](${source.url})` : `- ${prefix}${source.label}`;
  }).join("\n");
}
function renderCard(draft, options = {}) {
  ...
  const frontmatter = (0, import_yaml.stringify)(
    {
      id,
  ...
  const evidence = normalized.evidence.length === 0 ? "_None._" : normalized.evidence.map((item) => `- ${item.claim} \u2014 ${item.sourceReference}`).join("\n");
  const selectedConnections = normalized.connections.filter((connection) => connection.selected);
  const connections = selectedConnections.length === 0 ? "_None._" : selectedConnections.map((connection) => {
    const normalizedRef = connection.cardRef.replace(/\\/gu, "/");
    const filename = path2.posix.basename(normalizedRef).replace(/\.md$/iu, "");
...
function toStringArray(value) {
  if (Array.isArray(value)) {
    return value.filter((item) => ["string", "number", "boolean"].includes(typeof item)).map((item) => String(item).trim()).filter(Boolean);
  }
...
function markdownToText(markdown) {
  return markdown.replace(/```[\s\S]*?```/gu, " ").replace(/!\[[^\]]*\]\([^)]*\)/gu, " ").replace(/\[([^\]]+)\]\([^)]*\)/gu, "$1").replace(/\[\[([^\]|#]+)(?:#[^\]|]+)?(?:\|([^\]]+))?\]\]/gu, "$2 $1").replace(/<[^>]+>/gu, " ").replace(/[#>*_`~\-]+/gu, " ").replace(/\s+/gu, " ").trim();
}
...
function parseCard(markdown, cardRef) {
  const split = splitFrontmatter(markdown);
  const warnings = [...split.warnings];
  let metadata = {};
  if (split.frontmatter !== void 0) {
    try {
      const document = (0, import_yaml.parseDocument)(split.frontmatter, {
        prettyErrors: false,
        strict: false,
        uniqueKeys: false
      });
  ...
  const filename = path2.posix.basename(normalizedRef);
  ...
  const id = typeof metadata.id === "string" && metadata.id.trim() ? metadata.id.trim() : void 0;
  const createdAt = typeof metadata.created_at === "string" && metadata.created_at.trim() ? metadata.created_at.trim() : void 0;
  return {
    ...id ? { id } : {},
    title,
...
// src/core/types.ts
var DEFAULT_CARDS_DIRECTORY = "Patchouli";

// src/core/configuration.ts
async function validateCardsDirectory(vaultPath, cardsDirectory) {
  ...
  try {
    ...
    await fs2.access(resolved, fs2.constants.W_OK);
  } catch (error) {
    if (error instanceof PatchouliError) throw error;
...
function getDefaultConfigurationPath(environment = process.env) {
  const appData = environment.APPDATA;
  if (!appData || !path3.isAbsolute(appData)) {
  ...
  return path3.join(appData, "Patchouli", "configuration.json");
}
async function writeJsonAtomically(filePath, value) {
  const directory = path3.dirname(filePath);
  await fs2.mkdir(directory, { recursive: true });
  const temporaryPath = path3.join(directory, `.configuration-${randomUUID2()}.tmp`);
  let handle;
  try {
    handle = await fs2.open(temporaryPath, "wx", 384);
...
var ConfigurationStore = class {
  ...
  constructor(configurationPath = getDefaultConfigurationPath()) {
    this.configurationPath = path3.resolve(configurationPath);
  }
  ...
  async getStatus() {
    let raw;
    try {
      raw = await fs2.readFile(this.configurationPath, "utf8");
    } catch (error) {
    ...
    if (typeof stored !== "object" || stored === null || !("vaultPath" in stored) || !("cardsDirectory" in stored) || typeof stored.vaultPath !== "string" || typeof stored.cardsDirectory !== "string") {
...
function excerpt(bodyText, queryTerms) {
  if (bodyText.length <= 220) return bodyText;
  const folded = fold(bodyText);
  const positions = queryTerms.map((term) => folded.indexOf(term)).filter((position) => position >= 0);
...
var CardIndex = class {
  ...
  search(query, options = {}) {
    ...
    return results.sort(
      (left, right) => right.score - left.score || left.title.localeCompare(right.title, "en-US", { sensitivity: "base" }) || left.cardRef.localeCompare(right.cardRef, "en-US")
    ).slice(0, limit);
  }
};
function listCategories(cards) {
  const categories = /* @__PURE__ */ new Map();
  for (const card of cards) {
    const seenForCard = /* @__PURE__ */ new Set();
  ...
  return [...categories.values()].sort(
    (left, right) => left.name.localeCompare(right.name, "en-US", { sensitivity: "base" })
  );
}
...
var defaultAtomicCreateOperations = {
  async promote(temporaryPath, destinationPath) {
    await fs3.link(temporaryPath, destinationPath);
  }
};
async function atomicCreateFile(destinationPath, contents, operations = defaultAtomicCreateOperations) {
  const directory = path4.dirname(destinationPath);
  const temporaryPath = path4.join(directory, `.patchouli-${randomUUID3()}.tmp`);
  let handle;
  let promoted = false;
  try {
    handle = await fs3.open(temporaryPath, "wx", 384);
    ...
    await handle?.close().catch(() => void 0);
    await fs3.unlink(temporaryPath).catch((error) => {
      if (!promoted && (!isNodeError(error) || error.code !== "ENOENT")) throw error;
    });
  }
}
async function scanDirectory(directory, vaultRoot, cards) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      const resolved = await fs3.realpath(candidate);
      ...
      const markdown = await fs3.readFile(resolved, "utf8");
      cards.push(parseCard(markdown, toPosixRelativePath(vaultRoot, resolved)));
    }
  }
}
var VaultCardEngine = class {
  configurationStore;
  #index = new CardIndex([]);
  constructor(options = {}) {
  ...
  async scanCards() {
    ...
    try {
      const stats = await fs3.stat(cardsDirectory);
      if (!stats.isDirectory()) {
  ...
  async getCard(cardRef) {
    ...
    const stats = await fs3.stat(resolved);
    ...
    return parseCard(await fs3.readFile(resolved, "utf8"), toPosixRelativePath(configuration.vaultPath, resolved));
  }
  async writeCard(draft, options = {}) {
    const configuration = await this.configurationStore.requireConfiguration();
    ...
    const destinationPath = path4.join(cardsDirectory, rendered.draft.filename);
    ...
    try {
      await fs3.lstat(destinationPath);
      throw new PatchouliError("COLLISION", "A card with this title-derived filename already exists.", {
        filename: rendered.draft.filename
      });
    } catch (error) {
