...
function checkpointPrompt(transcript: string, previousDrafts: CardDraft[]): string {
  return [
    ...
    "First inventory independently reusable concepts (maximum 8 drafts). Split a substantive domain transition, a more general prerequisite, or an important foundational concept with its own mechanism/constraints/retrieval value, even if closely related to the original topic. Do not assume one conversation means one card. Do not split every named term, an incidental example, or tightly coupled derivation steps.",
    "For example, developed Unicode/UTF-8 knowledge is a general prerequisite distinct from NLP tokenization; a substantive Transformer explanation can deserve its own foundational card. A passing name alone does not. Use each concept's own categories.",
    "Each draft needs a concise Summary, durable Core knowledge in detailMarkdown, and optional examples or version-dependent figures in fyiMarkdown (empty string when none). Preserve mechanisms, qualifications, math and derivations in Core; do not demote an essential explanation to FYI.",
    "Keep an entire worked exercise in FYI: its instance-specific numeric targets, sample sentence and exercise-specific verification code belong together there, even if the general validation method deserves its own card. Core can explain the general algorithm or use symbolic parameters. For example, a 20-word/10-containing-s exercise and GPT vocabulary figures are FYI, not Core mechanisms.",
