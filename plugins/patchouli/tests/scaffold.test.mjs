...
import { mcpConfiguration } from "../scripts/runtime-config.mjs";
...
test("MCP configuration uses the build platform launcher and retains the Windows runtime locator", async () => {
  const configuration = JSON.parse(
    await readFile(path.join(pluginRoot, ".mcp.json"), "utf8"),
  );
  const server = configuration.mcpServers.patchouli;

  assert.deepEqual(configuration, mcpConfiguration(process.env.PATCHOULI_BUILD_PLATFORM || process.platform));
  assert.deepEqual(mcpConfiguration("win32").mcpServers.patchouli.args.slice(-2), [
...
test("packages trusted pre-compaction and task-cleanup hooks through the bundled runtime", async () => {
  ...
  assert.match(preCompact.hooks[0].command, /launch-patchouli-hook\.sh/u);
