...
3. Call `get_card` for the most relevant results before answering. Read enough cards to support the material claims; do not read unrelated results merely to increase coverage.
   Distinguish Core mechanisms from FYI examples or model/version figures. Follow a relevant prerequisite link when it supports the answer; retrieve the target rather than assuming its content. Legacy Detail remains valid readable evidence.
...
Inquiry is read-only. Do not call preview or save/update tools unless the user separately asks to capture new knowledge.
