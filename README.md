...
Stage 4 exposes all eight public MCP tools on top of the Stage 3 vault engine. `preview_card` returns normalized review data, a collision check, an expiring single-use token, conversational confirmation instructions, and the optional React MCP App resource. `save_card` accepts an edited final draft only after confirmation, preserves create-only vault semantics, and returns the original saved result for identical retries instead of creating a duplicate. Abandoned preview tokens expire after 15 minutes.
